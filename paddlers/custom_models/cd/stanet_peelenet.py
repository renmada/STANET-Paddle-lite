...
from .backbones import peelenet
...
class STANet(nn.Layer):
    ...
    def __init__(self, in_channels, num_classes, att_type='BAM', ds_factor=1, pretrained=None):
        super(STANet, self).__init__()

        WIDTH = 64

        self.extract = build_feat_extractor(in_ch=in_channels, width=WIDTH, pretrained=pretrained)
...
def build_feat_extractor(in_ch, width, pretrained):
    return nn.Sequential(Backbone(in_ch, 'peelenet', pretrained), Decoder(width))
...
class Backbone(nn.Layer, KaimingInitMixin):
    def __init__(self, in_ch, arch, pretrained_backbone=None):
        ...
        if arch == 'peelenet':
            ...
            if pretrained_backbone:
                ...
                with paddle.no_grad():
                    print(self.peelenet.parameters()[0].flatten()[:3])
        # if in_ch != 3:
    ...
    def forward(self, x):
        return self.peelenet(x)


class Decoder(nn.Layer, KaimingInitMixin):
    def __init__(self, f_ch):
        super(Decoder, self).__init__()
        self.dr1 = Conv1x1(128, 96, norm=True, act=True)
        self.dr2 = Conv1x1(256, 96, norm=True, act=True)
        self.dr3 = Conv1x1(512, 96, norm=True, act=True)
        self.dr4 = Conv1x1(704, 96, norm=True, act=True)
        self.conv_out = nn.Sequential(
            Conv3x3(
