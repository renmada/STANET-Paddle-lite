...
class PeleeNet(nn.Layer):
    ...
    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
                 num_init_features=32, bottleneck_width=[1, 2, 4, 4], drop_rate=0.05, class_num=1000):
        ...
        # ]))
        self.features = nn.Sequential(_StemBlock(3, num_init_features))
        ...
        for i, num_layers in enumerate(block_config):
            ...
            self.features.add_sublayer('transition%d' % (i + 1), BasicConv2d(
                num_features, num_features, kernel_size=1, stride=1, padding=0))

            if i != len(block_config) - 1:
                self.features.add_sublayer('transition%d_pool' % (i + 1), nn.AvgPool2D(kernel_size=2, stride=2))
...
if __name__ == '__main__':
    ...
    # paddle.summary(model, input=paddle.rand([1, 3, 224, 224]))
